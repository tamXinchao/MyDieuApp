...
public class ExcelService {
	...
	public byte[] exportToExcel(List<Product> products) {
		...
		headerStyle.setVerticalAlignment(VerticalAlignment.CENTER); // Correctly set the vertical alignment
		...
		dataStyle.setVerticalAlignment(VerticalAlignment.CENTER); // Correctly set the vertical alignment
	...
	private void createHeaderRow(Row row, CellStyle headerStyle) {
		String[] headers = { "STT", "Tên Sản Phẩm", "Xuất Xứ", "Chất Liệu", "Thể Loại", "Thương Hiệu", "Miêu tả",
				"Ngày Nhập" };
	...
	private void createProductRow(Product product, Row row, int count, CellStyle dataStyle) {
		...
		cell = row.createCell(1); // Ô thứ hai cho tên sản phẩm
		cell.setCellValue(product.getProductName());
		cell.setCellStyle(dataStyle);

		cell = row.createCell(2); // Ô thứ ba cho xuất xứ
		cell.setCellValue(product.getOrigin());
		cell.setCellStyle(dataStyle);

		cell = row.createCell(3); // Ô thứ tư cho chất liệu
		...
		cell = row.createCell(5); // Ô thứ năm cho tên thương hiệu
		cell.setCellValue(product.getManufacturer().getManuName());
		cell.setCellStyle(dataStyle);
		
		cell = row.createCell(6); // Ô thứ sáu cho ngày nhập
		cell.setCellValue(product.getDescribe());
		cell.setCellStyle(dataStyle);
		
		cell = row.createCell(7); // Ô thứ sáu cho ngày nhập
		cell.setCellValue(simpleDateFormat.format(product.getNgayNhap()));
		cell.setCellStyle(dataStyle);
